...
subplot(1,2,2); imshow(imreconstruct(imerode(Calculator_diff, ones(1,11)), Calculator_diff)); title('Odblaski pionowe');

%%
% to chyba zle
Calculator_dilate = imdilate(Calculator_diff, ones(1,21));
figure;
subplot(1,3,1); imshow(Calculator); title('Original');
subplot(1,3,2); imshow(Calculator_dilate); title('Dylatacja');
subplot(1,3,3); imshow(imreconstruct(min(Calculator_dilate, Calculator_reconstruct), Calculator_reconstruct));